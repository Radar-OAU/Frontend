...
const EventDetailsPage = () => {
  ...
  const eventDate = new Date(event.event_date_time || event.date);
  const isSoldOut = selectedCategory?.is_sold_out;
  
  // Calculate display price
  const activeCategories = event.ticket_categories?.filter(c => c.is_active && !c.is_sold_out) || [];
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 md:space-y-8 pb-20 px-4 pt-24">
      {/* Hero Section */}
